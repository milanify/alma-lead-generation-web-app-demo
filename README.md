...
- **Lightning Fast Sorting & Filtering:** When an admin searches for a lead or sorts by date, it happens instantly (in 0 milliseconds). We process the search text and sorting commands *locally* using React's `useMemo` algorithmic loops, altogether avoiding slow network requests back to the server.
